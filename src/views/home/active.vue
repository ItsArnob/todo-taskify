<template>
    <div class="relative">
        ...
        <transition-group name="list">
            <todoCard
                :activeCard="true"
                v-for="todo in allActiveTodoLists"
                :key="todo.id"
                :id="todo.id"
                :title="todo.name"
        ...
        <p
            v-if="todoListsLoaded && !allActiveTodoLists.length"
            class="text-center text-lg"
        >
            You dont have any active lists.
        </p>
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...mapGetters(["allActiveTodoLists", "todoListsLoaded"]),
    },
    methods: {
        getDate(date) {
            let dueByDate = date;
